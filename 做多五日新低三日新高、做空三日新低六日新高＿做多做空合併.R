library(quantmod)
par(mfrow=c(3,2))
STK= na.omit(get(getSymbols("KODK")))#4904.TW
#chartSeries(STK)
...
PZ=0
row1=Cl(STK) > runMax(lag(Hi(STK)),2)
row2=Cl(STK) < runMin(lag(Lo(STK)),4)
for (m in 5:nrow(STK)){
...
long=0
row1=Cl(STK) < runMin(lag(Lo(STK)),5)
row2=Cl(STK) > runMax(lag(Hi(STK)),2)
for (m in 7:nrow(STK)){
  PZ[m]=PZ[m-1]
  if (row1[m]==1){
...
PZ=0
row3=Cl(STK) > runMax(lag(Hi(STK)),6)
row4=Cl(STK) < runMin(lag(Lo(STK)),3)
for (m in 7:nrow(STK)){
