...
for (m in 21:nrow(STK)){
  ...
  if (PZ[m-1]!=0 && row2[m]==1){
    PL[m]=as.numeric(Cl(STK)[m]*PZ[m]-long)
    PZ[m]=long=0
  }
}
DD=cumsum(PL)-cummax(cumsum(PL))
...
View(cbind(as.numeric(Cl(STK)),as.numeric(Lo(STK))
           ,as.numeric(runMin(lag(Lo(STK)),5)),as.numeric(row1)
           ,as.numeric(Hi(STK))
...
PZ=0
row1=Cl(STK) >= BBands(Cl(STK))[,3]
row2=Cl(STK) <= BBands(Cl(STK))[,1]
for (m in 21:nrow(STK)){
  if (PZ==0 && row1[m-1]==0 && row1[m]==1){
...
for (m in 21:nrow(STK)){
  PZ[m]=PZ[m-1]
  if (row1[m]==1){
    short=short+as.numeric(Cl(STK)[m])
    PZ[m]=PZ[m-1]+1
  }
  if (PZ[m-1]!=0 && row2[m]==1){
    PL1[m]=as.numeric(Cl(STK)[m]*PZ[m]-short)
    PZ[m]=short=0
  }
}
DD=cumsum(PL1)-cummax(cumsum(PL1))
...
sum(PL1)
length(PL1[PL1>0])/length(PL1[PL1!=0])
mean(PL1[PL1>0])/abs(mean(PL[PL1<0]))
sum(PL1[PL1>=0])/abs(sum(PL1[PL1<0]))
View(cbind(as.numeric(Cl(STK)),as.numeric(Lo(STK))
           ,as.numeric(runMin(lag(Lo(STK)),5)),as.numeric(row1)
           ,as.numeric(Hi(STK))
...
plot(DD,type="h",col="darkgreen",ylim=yRang)
par(new=T)
plot(cumsum(PL2),col="orange",lwd=2,type="h",ylim=yRang)
...
for (m in 21:nrow(STK)){
  ...
  if (PZ[m-1]!=0 && row2[m]==1){
    PL2[m]=as.numeric(Cl(STK)[m]*PZ[m]-long)
    PZ[m]=long=0
  }
}
...
for (m in 21:nrow(STK)){
  ...
  if (PZ[m-1]!=0 && row4[m]==1){
    PL2[m]=PL2[m]+as.numeric(Cl(STK)[m]*PZ[m]-short)
    PZ[m]=short=0
  }
}
DD=cumsum(PL2)-cummax(cumsum(PL2))
